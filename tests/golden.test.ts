...
test.skipIf(!RUN)(
  "claude-pty text output matches claude -p text output",
  async () => {
...
test.skipIf(process.env.CLAUDE_PTY_E2E !== "1")(
  "stream-json event types match claude -p shape",
  async () => {
    const prompt = "Reply with exactly the word: pong";
...
test.skipIf(process.env.CLAUDE_PTY_E2E !== "1")(
  "bad API key yields non-zero exit + is_error",
  async () => {
    ...
    // and exits non-zero.
    const p = Bun.spawn(
      ["bun", "run", "src/main.ts", "--output-format", "json", "hi"],
      {
        env: {
          ...process.env,
          ANTHROPIC_API_KEY: "sk-bad-key",
          CLAUDE_PTY_TURN_TIMEOUT_MS: "20000",
        },
      },
    );
...
test.skipIf(process.env.CLAUDE_PTY_E2E !== "1")(
  "permission box is auto-denied; run terminates without hanging",
  async () => {
    const p = Bun.spawn(
      [
        "bun",
        ...
        "--permission-mode",
        "default",
        "Run the shell command: git status",
      ],
      { env: { ...process.env, CLAUDE_PTY_TURN_TIMEOUT_MS: "60000" } },
    );
    const out = await new Response(p.stdout).text();
    const _code = await p.exited;
...
test.skipIf(process.env.CLAUDE_PTY_E2E !== "1")(
  "--continue/--resume recalls prior conversation",
  async () => {
    ...
    const rawJson = await new Response(
      Bun.spawn(
        [
          ...
          "--output-format",
          "json",
          "Remember the codeword: platypus",
        ],
        spawnEnv,
      ).stdout,
    ).text();
...
test.skipIf(process.env.CLAUDE_PTY_E2E !== "1")(
  ...
  async () => {
    const ndjson =
      JSON.stringify({
        type: "user",
        content: "Remember the number 7. Reply OK.",
      }) +
      "\n" +
      JSON.stringify({
        type: "user",
        content: "What number did I say? Reply with just the number.",
      }) +
      "\n";
    const p = Bun.spawn(
      [
...
test.skipIf(process.env.CLAUDE_PTY_E2E !== "1")(
  "--json-schema yields structured_output via TUI (no -p)",
  async () => {
    const schema =
