...
import { parseLine, parseTranscript } from "../../src/domain/transcript";

test("parseLine ignores meta/system lines", () => {
  expect(parseLine('{"type":"system","subtype":"bridge_status"}').kind).toBe(
    "ignored",
  );
...
test("parseLine extracts an assistant message", () => {
  const line =
    '{"type":"assistant","message":{"model":"claude-opus-4-8","content":[{"type":"text","text":"hi"}],"stop_reason":"end_turn","usage":{"input_tokens":5,"output_tokens":2,"cache_creation_input_tokens":0,"cache_read_input_tokens":0}},"uuid":"x","timestamp":"2026-06-08T10:00:00.000Z"}';
